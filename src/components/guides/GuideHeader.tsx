 ...
 */
const GuideHeader: React.FC<GuideHeaderProps> = ({ guide, averageRating, recommendedDays, price }) => {
  ...
  return (
    <div className="mb-6">
      <div className="flex flex-wrap justify-between items-start gap-4">
        <div>
          ...
          <div className="flex items-center mt-2 space-x-4">
            <div className="flex items-center">
              ...
              <span className="ml-2 text-sm text-gray-600 dark:text-gray-300">
                {averageRating > 0 ? `${averageRating.toFixed(1)} (${guide.reviews_count})` : "Brak ocen"}
              </span>
            </div>
          ...
          <div className="flex items-center mt-2 text-sm text-gray-600 dark:text-gray-300">
            ...
            <span>
              {recommendedDays} {recommendedDays === 1 ? "dzień" : recommendedDays < 5 ? "dni" : "dni"}
            </span>
          </div>
        </div>
      ...
      <div className="mt-4 flex items-center">
        ...
        <span className="ml-2 text-sm text-gray-600 dark:text-gray-300">{guide.location_name}</span>
      </div>
    </div>
  );
};
