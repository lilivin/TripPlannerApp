...
export const guidesService = {
  async getGuides(
    ...
    if (countError) {
      throw new ApiError(
        ...
        'Błąd podczas pobierania liczby przewodników',
        countError
      );
    }
    ...
    if (error) {
      ...
      throw new ApiError(
        ApiErrorTypes.INTERNAL_ERROR,
        'Błąd podczas pobierania przewodników',
        error
      );
    }
    ...
    let averageRatings: Record<string, number | null> = {};
  ...
  async getGuideDetails(
    ...
    if (error) {
      ...
      throw new ApiError(
        ApiErrorTypes.INTERNAL_ERROR,
        'Błąd podczas pobierania szczegółów przewodnika',
        error
      );
    }
    ...
    if (includeAttractions) {
      try {
        ...
        if (attractionsError) {
          ...
          let guideAttractionsMap: Record<string, {
            order_index: number;
            custom_description: string | null;
            is_highlight: boolean;
          }> = {};
          ...
          let attractionTags: Record<string, TagDto[]> = {};
